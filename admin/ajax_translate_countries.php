...
// 1. Get countries with missing translation
$sql = "SELECT id, name FROM countries WHERE name_ar IS NULL OR name_ar = '' OR name_en IS NULL OR name_en = ''";
$stmt = $pdo->query($sql);
$items = $stmt->fetchAll(PDO::FETCH_ASSOC);

if (empty($items)) {
    echo json_encode(['success' => true, 'message' => 'No countries found needing translation.', 'count' => 0]);
    exit;
}
...
function translateWithGemini($dataToTranslate, $apiKey) {
    ...
    $prompt = "Translate the following country names into English and Arabic. 
...
foreach ($translations as $trans) {
    if (isset($trans['id'], $trans['name_en'], $trans['name_ar'])) {
        $stmt = $pdo->prepare("UPDATE countries SET name_en = ?, name_ar = ? WHERE id = ?");
...
echo json_encode(['success' => true, 'message' => "Successfully translated $updatedCount countries.", 'count' => $updatedCount]);
