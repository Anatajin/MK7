...
// 1. Get players with missing translation
$sql = "SELECT p.id, p.name, t.name as team_name 
        FROM players p 
        LEFT JOIN teams t ON p.team_id = t.id 
        WHERE p.name_ar IS NULL OR p.name_ar = '' OR p.name_en IS NULL OR p.name_en = ''
        LIMIT 100"; // Limit to avoid too large prompt
$stmt = $pdo->query($sql);
$items = $stmt->fetchAll(PDO::FETCH_ASSOC);

if (empty($items)) {
    echo json_encode(['success' => true, 'message' => 'No players found needing translation.', 'count' => 0]);
    exit;
}
...
function translateWithGemini($dataToTranslate, $apiKey) {
    ...
    $prompt = "Translate the following football player names into English and Arabic. 
...
foreach ($translations as $trans) {
    if (isset($trans['id'], $trans['name_en'], $trans['name_ar'])) {
        $stmt = $pdo->prepare("UPDATE players SET name_en = ?, name_ar = ? WHERE id = ?");
...
echo json_encode(['success' => true, 'message' => "Successfully translated $updatedCount players.", 'count' => $updatedCount]);
